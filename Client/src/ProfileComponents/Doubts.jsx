...
export default function Doubts() {
  ...
  useEffect(() => {
    ...
    const fetchDoubts = async () => {
      try {
        const response = await fetch(`${apiUrl}/api/v1/profile/getUserDoubts`, {
          method: 'GET',
          headers: {
    ...
    <div className="flex-[0.2] ml-24 w-3/5">
      ...
      {doubts.slice().reverse().map((doubt) => (
        <Post
          ...
          accountType={doubt.createdBy.accountType}
          userId={userId} // Assuming the field is named profilePicture
        />
      ))}

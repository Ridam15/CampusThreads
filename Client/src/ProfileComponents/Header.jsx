...
function Header() {
  ...
  useEffect(() => {
    ...
    const fetchSentRequests = async () => {
      try {
        const response = await fetch(`${apiUrl}/api/v1/friends/getallreqsent`, {
          method: 'GET',
          headers: {
            'Content-Type': 'application/json',
            'Authorization': 'Bearer ' + token,
  ...
  const handleSearchInputChange = async (event) => {
    ...
    if (query) {
      setLoading(true);
      try {
        const response = await fetch(`${apiUrl}/api/v1/search/searchuser/${query}`, {
          method: 'GET',
          headers: {
            'Content-Type': 'application/json',
            'Authorization': 'Bearer ' + token,
  ...
  const handleSendFriendRequest = async (userId) => {
    const token = localStorage.getItem("Token");
    try {
      const response = await fetch(`${apiUrl}/api/v1/friends/sendreq`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': 'Bearer ' + token,
      ...
      if (data.success) {
        ...
        const friendsResponse = await fetch(`${apiUrl}/api/v1/friends/getAllFriends`, {
          method: 'GET',
          headers: {
            'Content-Type': 'application/json',
            'Authorization': 'Bearer ' + token,
    ...
    <div className="header">
      ...
      <div className="header__right">
        <div className="header__right__home">
          <Link to="/Home">
            <HeaderOption Icon={HomeIcon} title="Home" active={location.pathname === '/Home'} />
          </Link>
        </div>

        <div className="header__right__community">
          <Link to="/Community">
            <HeaderOption Icon={GroupsIcon} title="Community" active={location.pathname === '/Community'} />
          </Link>
        </div>

        <div className="header__right__qa">
          <Link to="/Q&A">
            <HeaderOption Icon={QuestionAnswerIcon} title="Q&A" active={location.pathname === '/Q&A'} />
          </Link>
        </div>

        <div className="header__right__profile">
          <Link to="/Profile">
            <HeaderOption avatar={picture} title="Profile" active={location.pathname === '/Profile'} />
          </Link>
        </div>

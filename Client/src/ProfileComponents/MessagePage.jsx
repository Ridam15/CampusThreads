...
function MessagePage({ firstName, onClose }) {
    ...
    const createUserIfNotExists = async (userId, nickname, friendProfileUrl) => {
        ...
        try {
            ...
            if (!response.ok) {
                if (response.status === 400 && responseData.code === 400202) {
    ...
    const startChat = async (friend) => {
        ...
        if (sendbirdInstance) {
            try {
                ...
                const friendProfileUrl = friend.profilePicture || 'https://example.com/default-profile.png';
                await createUserIfNotExists(userId, firstName, friendProfileUrl);
                await createUserIfNotExists(friend.id, `${friend.firstName}`, friendProfileUrl);
                            ...
                            <div>
                                ...
                                <div className="mb-4">
                                    {messages.slice().reverse().map((message) => (
                                        (message?._sender?.userId === selectedFriend.id) ?

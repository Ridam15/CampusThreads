...
export default function ProfileCard({ communityDetails, onUpdateButtonClick }) {
  ...
  const handleClick = async () => {
    try {
      ...
      if (!isUserMember) {
        ...
        await fetch('http://localhost:3000/api/v1/community/addMember', {
          method: 'POST',
          headers: {
        ...
        await fetch('http://localhost:3000/api/v1/community/removeMember', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
            'Authorization': 'Bearer ' + token,
  ...
  const handleConfirmDelete = async () => {
    try {
      const token = localStorage.getItem("Token");
      const response = await fetch('http://localhost:3000/api/v1/community/delete', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': 'Bearer ' + token,
